...
function labelReady(){return !!labelImage&&VL.imgLoaded&&VL.editMode&&!(VL.lblServerMode?VL.lblServerPlaying:(LV&&!LV.paused));}
...
function labelRender(){
 ...
 const zoom=labelDisplayScale();VL.scale=Math.min(1,zoom);
 const dw=Math.max(1,Math.round(labelImage.width*VL.scale)),dh=Math.max(1,Math.round(labelImage.height*VL.scale));
...
function labelZoom(value){
 if(!labelReady()||VL.drawing)return;
...
function labelTogglePan(){if(!labelReady())return;VL.drawing=false;VL.drawCur=null;Dctx.clearRect(0,0,DC.width,DC.height);labelPan=!labelPan;labelControls();}
document.addEventListener('DOMContentLoaded',()=>{
 const wrap=document.getElementById('canvasWrap');
 DC.addEventListener('mousedown',e=>{
  if(!labelReady())return;
  if(e.button===1||(e.button===0&&labelPan)){
   e.preventDefault();e.stopImmediatePropagation();VL.drawing=false;
 ...
 window.addEventListener('blur',()=>{labelDrag=null;VL.drawing=false;VL.drawCur=null;Dctx.clearRect(0,0,DC.width,DC.height);});
